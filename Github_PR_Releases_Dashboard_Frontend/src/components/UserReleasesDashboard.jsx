...
const UserReleasesDashboard = () => {
  ...
  useEffect(() => {
    fetch("/user_names.json").then((res) => res.json()).then(setAllUsers).catch(() => setAllUsers([]));
    fetch("/repo_names.json").then((res) => res.json()).then(setAllRepos).catch(() => setAllRepos([]));
    fetch("/release_tags.json").then((res) => res.json()).then(setAllTags).catch(() => setAllTags([]));
    // ✅ Load POD map
    fetch("/pod_names.json").then((res) => res.json()).then(setPodMap).catch(() => setPodMap({}));
  }, []);
  ...
  useEffect(() => {
    ...
    if (podSearch.trim() && podMap[podSearch]) {
      ...
      data = data.filter((r) => svc.includes((r.repo_name || "").toLowerCase()));
    }
    ...
    setPage(1);
  }, [allReleases, tagSearch, repoSearchTable, username, fromDate, toDate, sortConfig, podSearch, podMap]);
  ...
  // ✅ Filtered POD keys for dropdown
    ...
    <div className="container mt-4" style={{ backgroundColor: darkMode ? "#121212" : "#ffffff" }}>
      ...
      <Form>
        <Row className="align-items-center mb-2">
          ...
          {/* Tag autocomplete — ✅ fixed: now uses tagDropdownRef (was wrongly using userDropdownRef) */}
          <Col md={3} ref={tagDropdownRef} style={{ position: "relative" }}>
            <Form.Control
                ...
                {allTags
                  ...
                  .map((name, idx) => (
                    <div
                      key={idx}
                      className="dropdown-item"
                      onClick={() => { setTagSearch(name); setShowTagDropdown(false); }}
                      style={{ ...formControlStyle, cursor: "pointer" }}
                    >
                      {name}
                    </div>
      ...
      {loading ? (
        ...
        <>
          ...
          >
            ...
            >
              <tr>
                <th onClick={() => handleSort("tag_name")}  style={{ cursor: "pointer", color: darkMode ? "#ffffff" : "#000000" }}>Name / Tag</th>
                <th onClick={() => handleSort("repo_name")} style={{ cursor: "pointer", color: darkMode ? "#ffffff" : "#000000" }}>Repo</th>
                <th onClick={() => handleSort("author")}    style={{ cursor: "pointer", color: darkMode ? "#ffffff" : "#000000" }}>Author</th>
                <th onClick={() => handleSort("created_at")}style={{ cursor: "pointer", color: darkMode ? "#ffffff" : "#000000" }}>Created At</th>
              </tr>
            </thead>
            <tbody>
...
//   return (
//     <div className="container mt-4" style={{ backgroundColor: darkMode ? "#121212" : "#ffffff" }}>
//       <h2 className="text-center mb-4" style={{ color: darkMode ? "#ffffff" : "#000000" }}>GitHub Release Catalog</h2>
//       <Form>
//         <Row className="align-items-center mb-2">
